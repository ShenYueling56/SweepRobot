...
#include <stdint.h>
#include "iostream"
#define LABEL_NALE_TXT_PATH "/data/ttt/initial/robot_labels_list_5.txt"
#define OUTPUTS_PATH "/data/ttt/model_output_data.txt"


static char *labels[OBJ_CLASS_NUM];

const int anchor0[6] = {29, 18, 28, 31, 41, 26};
const int anchor1[6] = {47, 45, 59, 19, 75, 31};
const int anchor2[6] = {86, 71, 142, 32, 251, 74};
...
static int saveOutput(const char *file_name, float *input,  int grid_h, int grid_w, uint8_t zp, float scale)          //float *output, int element_size)
{
    FILE *fp;
    fp = fopen(file_name, "a+");
    printf("start    saving     outputs\n");
    int tensor_size = grid_h * grid_w * 27;       //(5+4)*3
    ...
    for (int i = 0; i < grid_h; i++)
    {
        for (int j = 0; j < grid_w; j++)
        {
            for (int c = 0; c < 30; c++)
            {
                fprintf(fp, "%.6f ", input[grid_w * i + j + grid_w * grid_h * c]);
                fprintf(fp, "\n");
            }
        }
    }
...
//    uint8_t thres_u8 = qnt_f32_to_affine(thres, zp, scale);
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                float box_conf_f32 = input[(PROP_BOX_SIZE * a + 4) * grid_len + i * grid_w + j];
//                std::cout << "origin float conf" << box_conf_f32 << std::endl;
                float box_confidence = sigmoid(box_conf_f32);
...
//                    float box_conf_f32 = sigmoid(deqnt_affine_to_f32(box_confidence, zp, scale));
                    ...
                    {
//                        if (k == 4 && (box_w * box_h < 7000)) continue;

                        float prob = in_ptr[(5 + k) * grid_len];
//                        std::cout << "class: " << k << "    prob: " << sigmoid(prob) << std::endl;
...
//                    if (maxClassId == 1) maxClassId = 4;
//                    std::cout << "The best class: " << maxClassId << "    prob: " << sigmoid(maxClassProbs) << std::endl;
...
//    saveOutput(OUTPUTS_PATH, input0, grid_h0, grid_w0, qnt_zps[0], qnt_scales[0]);
...
//    saveOutput(OUTPUTS_PATH, input1, grid_h1, grid_w1, qnt_zps[1], qnt_scales[1]);
...
//    saveOutput(OUTPUTS_PATH, input2, grid_h2, grid_w2, qnt_zps[2], qnt_scales[2]);
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        group->results[last_count].obj_name = id;
