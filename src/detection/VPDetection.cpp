#include "detection/VPDetection.h"
#include "time.h"
#include <iostream>
...
void VPDetection::run(std::vector<std::vector<double> > &lines, cv::Point2d pp, double f, std::vector<cv::Point3d> &vps,
...
void VPDetection::getVPHypVia2Lines(std::vector<std::vector<cv::Point3d> > &vpHypo) {
    ...
    for (int i = 0; i < it; ++i) {
        ...
        cv::Mat_<double> vp1 = (cv::Mat_<double>(3, 1) << vp1_Img(0) / vp1_Img(2) - pp.x, vp1_Img(1) / vp1_Img(2) -
                                                                                          pp.y, f);
...
void VPDetection::getSphereGrids(std::vector<std::vector<double> > &sphereGrid) {
    ...
    for (int i = 0; i < lines.size() - 1; ++i) {
        for (int j = i + 1; j < lines.size(); ++j) {
            ...
            sphereGrid[LA][LO] += sqrt(lineInfos[i].length * lineInfos[j].length) *
                                  (sin(2.0 * angleDev) + 0.2); // 0.2 is much robuster
        }
    }
...
void VPDetection::getBestVpsHyp(std::vector<std::vector<double> > &sphereGrid,
                                std::vector<std::vector<cv::Point3d> > &vpHypo, std::vector<cv::Point3d> &vps) {
    int num = vpHypo.size();
