...
#include "include/orb_slam3/MapPoint.h"
#include "include/orb_slam3/KeyFrame.h"
#include "include/orb_slam3/ORBextractor.h"
...
//Copy Constructor
    ...
    Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor *extractorLeft,
                 ...
                 cv::Mat &K, cv::Mat &K_left, cv::Mat &distCoef_left, cv::Mat &R_left, cv::Mat &P_left,
            ...
            : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft), mpORBextractorRight(extractorRight),
              ...
              mK(K.clone()), mK_ori(K_left.clone()), mDistCoef(distCoef_left.clone()), mR(R_left.clone()),
              mP(P_left.clone()),
              mK_right(K_right.clone()), mDistCoef_right(distCoef_right.clone()), mR_right(R_right.clone()),
              mP_right(P_right.clone()), mbf(bf), mThDepth(thDepth),
        ...
        N = mvKeys.size();
        N_R = mvKeysRight.size();
        mvpMapPoints = vector<MapPoint *>(N, static_cast<MapPoint *>(NULL));
        if (mvKeys.empty() || mvKeysRight.empty())
            return;
        ...
        double time1 = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t5 - t4).count();
        double time2 = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t6 - t5).count();
//        std::cout << "time1: " << time1 << " time2: " << time2 << std::endl;
        ...
        mTimeORB_Ext = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t2 - t1).count();
        mTimeStereoMatch = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(t3 - t2).count();
    ...
    vector<size_t>
    Frame::GetFeaturesInArea(const float &x, const float &y, const float &r, const int minLevel, const int maxLevel,
    ...
    void Frame::GetUnMatchedKPbyBucketing(const Frame *pFrame, std::vector<unsigned int> &vUnMatchedKeyPoints) {
        ...
        for (size_t i = 0; i < bucket_cols * bucket_rows; i++) {
            if (buckets[i].matched_size < min_matched_features) {
                vUnMatchedKeyPoints.insert(vUnMatchedKeyPoints.end(), buckets[i].kp_unmatched_indices.begin(),
                                           buckets[i].kp_unmatched_indices.end());
            }
        }
        ...
        for (int iL:unmatched) {
            ...
            if (bestDist < thOrbDist) {
                ...
                if (disparity >= minD && disparity < maxD) {
                    ...
                    mvuRight[iL] = (bestuR + disparity) - disparity / CAM_SCALE;
                    disparity = disparity / CAM_SCALE;
...
//                    转换到相机坐标系下Pc
    ...
    void Frame::PrepareStereoCandidates() {
    ...
    Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor *extractorLeft,
        ...
        double t_stereomatches = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                t3 - t2).count();
    ...
    void Frame::ComputeStereoFishEyeMatches() {
        ...
        for (vector<vector<cv::DMatch>>::iterator it = matches.begin(); it != matches.end(); ++it) {
            if ((*it).size() >= 2 && (*it)[0].distance < (*it)[1].distance * 0.7) {
                ...
                cv::Mat p3D;
                descMatches++;
                float sigma1 = mvLevelSigma2[mvKeys[(*it)[0].queryIdx +
                                                    monoLeft].octave], sigma2 = mvLevelSigma2[mvKeysRight[
                        (*it)[0].trainIdx + monoRight].octave];
                float depth = static_cast<KannalaBrandt8 *>(mpCamera)->TriangulateMatches(mpCamera2,
                                                                                          mvKeys[(*it)[0].queryIdx +
                                                                                                 monoLeft], mvKeysRight[
                                                                                                  (*it)[0].trainIdx +
                                                                                                  monoRight], mRlr,
