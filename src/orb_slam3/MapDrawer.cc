...
#include "include/orb_slam3/MapDrawer.h"
    ...
    void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph, const bool bDrawInertialGraph) {
        ...
        if (bDrawKF) {
            for (size_t i = 0; i < vpKFs.size(); i++) {
                ...
                unsigned int index_color = pKF->mnOriginMapId % 6;
                ...
                } else if (pKF->GetMap()->IsRecentInitKF(pKF)) {
                    glLineWidth(mKeyFrameLineWidth * 5);
            ...
            for (Map *pMap : vpMaps) {
                ...
                for (size_t i = 0; i < vpKFs.size(); i++) {
                    ...
                    if (!vpKFs[i]->GetParent()) // It is the first KF in the map
                    {
                        ...
                        glColor3f(1.0f, 0.0f, 0.0f);
                        glBegin(GL_LINES);
                    } else if (vpKFs[i]->GetMap()->IsRecentInitKF(vpKFs[i])) {
                        glLineWidth(mKeyFrameLineWidth * 5);
                        glColor3f(0.95f, 0.85f, 0.5f);
                        glBegin(GL_LINES);
                    } else {
