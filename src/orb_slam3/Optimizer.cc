...
#include "include/orb_slam3/OptimizableTypes.h"
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
                ...
                /*if(!vSE3->fixed())
            {
                ...
                if (dist > 1) {
                    ...
                    Verbose::PrintMess(
                            "GBA: KF " + to_string(pKF->mnId) + " had been moved " + to_string(dist) + " meters",
            ...
            if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
                ...
                if (pKFi->bImu && pKFi->mPrevKF->bImu) {
                    ...
                    if (!bInit) {
                        ...
                        egr->setVertex(0, VG1);
                        egr->setVertex(1, VG2);
                        cv::Mat cvInfoG = pKFi->mpImuPreintegrated->C.rowRange(9, 12).colRange(9, 12).inv(
                                cv::DECOMP_SVD);
                        ...
                        ear->setVertex(0, VA1);
                        ear->setVertex(1, VA2);
                        cv::Mat cvInfoA = pKFi->mpImuPreintegrated->C.rowRange(12, 15).colRange(12, 15).inv(
                                cv::DECOMP_SVD);
...
// accelerate track BA
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, vector<KeyFrame *> &vpNonEnoughOptKFs) {
        ...
        if (bDoMore) {
            ...
            Verbose::PrintMess("LM-LBA: First optimization has " + to_string(nMonoBadObs) + " monocular and " +
                               to_string(nStereoBadObs) + " stereo bad observations", Verbose::VERBOSITY_DEBUG);
    ...
    void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF) {
...
#ifdef LIMIT_LOCAL_BA
        int num_free_KF = lLocalKeyFrames.size();
        // sort KF according to timestamp; keep recent-N ones
        if (num_free_KF > GOOD_GRAPH_KF_THRES) {
            lLocalKeyFrames.sort([](const KeyFrame *pKF1, const KeyFrame *pKF2) {
        ...
        for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();
             lit != lend; lit++) {
            KeyFrame *pKFi = *lit;
            if (pKFi->mnId == pMap->GetInitKFid() || pMap->IsRecentInitKF(pKFi)) {
                num_fixedKF = 1;
            }
            vector<MapPoint *> vpMPs = pKFi->GetMapPointMatches();
            for (vector<MapPoint *>::iterator vit = vpMPs.begin(), vend = vpMPs.end(); vit != vend; vit++) {
        ...
        if (num_fixedKF < 2) {
            ...
            list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin();
            int lowerId = pKF->mnId;
            // 在这里要赋值为初始值，不然后面如果使用这个关键帧但这个关键帧没有被初始化，就会崩溃 fixme
            KeyFrame *pLowerKf = NULL;
            int secondLowerId = pKF->mnId;
            KeyFrame *pSecondLowerKF = NULL;

            for (; lit != lLocalKeyFrames.end(); lit++) {
                KeyFrame *pKFi = *lit;
                if (pKFi == pKF || pKFi->mnId == pMap->GetInitKFid() || pMap->IsRecentInitKF(pKFi)) {
                    continue;
                }
            ...
            if (pLowerKf) {
                //cout<<"fixed 2 kf: "<<pLowerKf->mnId<<endl;
                lFixedCameras.push_back(pLowerKf);
                lLocalKeyFrames.remove(pLowerKf);
                num_fixedKF++;
            }
            if (num_fixedKF < 2 && pSecondLowerKF) {
                //cout<<"fixed 3 kf: "<<pSecondLowerKF->mnId<<endl;
                lFixedCameras.push_back(pSecondLowerKF);
                lLocalKeyFrames.remove(pSecondLowerKF);
                num_fixedKF++;
            }
...
#ifdef LIMIT_LOCAL_BA
        int num_fixed_KF = lFixedCameras.size();
        // sort KF according to timestamp; keep recent-N ones
        if (num_fixed_KF > GOOD_GRAPH_FIXEDKF_THRES) {
            lFixedCameras.sort([](const KeyFrame *pKF1, const KeyFrame *pKF2) {
                return pKF1->mTimeStamp < pKF2->mTimeStamp;
            });
            while (num_fixed_KF > GOOD_GRAPH_FIXEDKF_THRES) {
                (*lFixedCameras.begin())->mnBAFixedForKF = pKF->mnId - 1;
                lFixedCameras.pop_front();
                num_fixed_KF--;
            }
            assert(lFixedCameras.size() == GOOD_GRAPH_FIXEDKF_THRES);
        }
#endif
        ...
        unsigned long maxKFid = 0;
        // Set Local KeyFrame vertices
        set<long unsigned int> graphKFs;
        for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();
             lit != lend; lit++) {
            KeyFrame *pKFi = *lit;
            g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
            vSE3->setEstimate(Converter::toSE3Quat(pKFi->GetPose()));
            vSE3->setId(pKFi->mnId);
            vSE3->setFixed(pKFi->mnId == pMap->GetInitKFid() || pMap->IsRecentInitKF(pKFi));
            optimizer.addVertex(vSE3);
            if (pKFi->mnId > maxKFid)
                maxKFid = pKFi->mnId;

            graphKFs.insert(pKFi->mnId);
        }
...
#ifdef LIMIT_LOCAL_BA
                if (nEdges > GOOD_GRAPH_EDGES_THRES)
                    break;
#endif
            }
        }

//    cout<< "LBA lLocalKeyFrames: " <<lLocalKeyFrames.size() << " lFixedCameras:" <<lFixedCameras.size()<<" "
//    <<"nEdges: "<<nEdges<<" mono_edges: "<<mono_edges<<" stereo_edges: "<<stereo_edges<<endl;
...
// accelerate local BA
        ...
        for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();
            ...
            if (dist > 1.0) {
                ...
                Verbose::PrintMess("LM-LBA: Good observations in mono " + to_string(numMonoMP) + " and stereo " +
                                   to_string(numStereoMP), Verbose::VERBOSITY_DEBUG);
    ...
    void Optimizer::OptimizeEssentialGraph(Map *pMap, KeyFrame *pLoopKF, KeyFrame *pCurKF,
        ...
        for (map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend = LoopConnections.end();
            ...
            for (set<KeyFrame *>::const_iterator sit = spConnections.begin(), send = spConnections.end();
                ...
                cout << nIDj << "-dis: " << fixed << setprecision(2) << Sji.translation().norm() << "; ";
        ...
        vector<KeyFrame *> orderedKFsInMap = pMap->GetAllOrderedKeyFrames();
        ...
        for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
            ...
            const int nIDi = pKF->mnId;
            bool isRecentInit = false;

            if (pMap->IsRecentInitKF(pKF))
                isRecentInit = true;
            ...
            if (pParentKF) {
                ...
                //if(isRecentInit)
                //    cout<<fixed<<setprecision(2)<<"parent edge cur: "<<nIDi<< " con: "<<nIDj<<" dis: "<<Sji.translation().transpose().norm()<<endl;

                //连接断点，并自适应权重
                if (isRecentInit) {
                    Eigen::Matrix<double, 7, 7> matLambda = Eigen::Matrix<double, 7, 7>::Identity();
                    //if(Sji.translation().transpose().norm()<0.05)
                    {
                        double info = 1000.0;
                        //if(Sji.translation().norm()!=0.0)
                        //    info = min(1.0/Sji.translation().norm(),10.0);
                        matLambda(3, 3) = info;
                        matLambda(4, 4) = info;
                        matLambda(5, 5) = info;
                    }
            ...
            for (set<KeyFrame *>::const_iterator sit = sLoopEdges.begin(), send = sLoopEdges.end();
                ...
                if (pLKF->mnId < pKF->mnId) {
                    ...
                    //if(isRecentInit)
                    //    cout<<fixed<<setprecision(2)<<"loop edge cur: "<<nIDi<< " con: "<<pLKF->mnId<<" dis: "<<Sli.translation().transpose().norm()<<endl;

                    //{
                    //    Eigen::Matrix<double,7,7> matLambda = Eigen::Matrix<double,7,7>::Identity();
                    //    double info = 10.0;
                    //    cout<<"loop norm: "<<Sli.translation().norm()<<endl;
                    //    if(Sli.translation().norm()!=0.0)
                    //        info = min(1.0/Sli.translation().norm(),1e3);
                    //    matLambda(0,0) = info;
                    //    matLambda(1,1) = info;
                    //    matLambda(2,2) = info;
                    //    matLambda(3,3) = info;
            ...
            for (vector<KeyFrame *>::const_iterator vit = vpConnectedKFs.begin(); vit != vpConnectedKFs.end(); vit++) {
                ...
                if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn)) {
                    ...
                    if (!pKFn->isBad() && pKFn->mnId < pKF->mnId) {
                        ...
                        //if(isRecentInit)
                        //    cout<<fixed<<setprecision(2)<<"covi edge cur: "<<nIDi<< " con: "<<pKFn->mnId<<" dis: "<<Sni.translation().transpose().norm()<<endl;
            ...
            if (isRecentInit && false) {
                vector<KeyFrame *>::iterator ret;
                ret = std::find(orderedKFsInMap.begin(), orderedKFsInMap.end(), pKF);
                if (ret == orderedKFsInMap.end()) {
                    cout << "not found" << endl;
                } else {
                    //cout << "found it" << endl;
                    if (ret != orderedKFsInMap.begin()) {
                        for (vector<KeyFrame *>::iterator iter = ret; iter != orderedKFsInMap.begin(); iter--) {
                            //获取其上一帧
                            KeyFrame *pKFn = *(iter - 1);
                            if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn)) {
                                if (!pKFn->isBad() && pKFn->mnId < pKF->mnId) {
                                    ...
                                    //if(isRecentInit)
                                    //{
                                    //    cout<<" addconnect "<< pKFn->mnId<<endl;
                                    //}
                                    //setInformation
                                    g2o::Sim3 Sni = Snw * Swi;
                                    //cout<<"keframe "<<nIDi<<". "<<endl;
                                    //cout<<"pKFn->mnId "<<pKFn->mnId<<". "<<endl;
                                    //cout<<"translation "<<Sni.translation().transpose()<<". "<<endl;
                                    //cout<<"translation norm"<<Sni.translation().norm()<<". "<<endl;

                                    g2o::EdgeSim3 *en = new g2o::EdgeSim3();
                                    en->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                                            pKFn->mnId)));
                                    en->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(
                                            nIDi)));
                                    en->setMeasurement(Sni);

                                    //Eigen::Matrix<double,7,7> matLambda = Eigen::Matrix<double,7,7>::Identity();
                                    //if(Sni.translation().transpose().norm()<0.05)
                                    //{
                                    //    double info = 10.0;
                                    //    if(Sni.translation().norm()!=0.0)
                                    //        info = min(1.0/Sni.translation().norm(),10.0);
                                    //    matLambda(3,3) = info;
                                    //    matLambda(4,4) = info;
                                    //    matLambda(5,5) = info;
                                    //}
    ...
    int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12,
        ...
        Verbose::PrintMess("Sim3: There are " + to_string(nCorrespondences) + " matches, " + to_string(nInKF2) +
                           " are in the KF and " + to_string(nOutKF2) + " are in the connected KFs. There are " +
                           to_string(nMatchWithoutMP) + " matches which have not an associate MP",
                           Verbose::VERBOSITY_DEBUG);
    ...
    int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1,
                                vector<KeyFrame *> &vpMatches1KF, g2o::Sim3 &g2oS12, const float th2,
                                const bool bFixScale, Eigen::Matrix<double, 7, 7> &mAcumHessian,
                                const bool bAllPoints) {
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolverX::LinearSolverType *linearSolver;
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (i2 < 0)
                Verbose::PrintMess("Sim3-OPT: Error, there is a matched which is not find it",
                                   Verbose::VERBOSITY_DEBUG);
    ...
    void
    Optimizer::InertialOptimization(vector<KeyFrame *> vpKFs, Eigen::Vector3d &bg, Eigen::Vector3d &ba, float priorG,
                                    float priorA) {
        int its = 200; // Check number of iterations
        ...
        /*for(map<int, int>::iterator it = mStatsObs.begin(); it != mStatsObs.end(); ++it)
    {
        ...
        if (!vToErase.empty()) {
            ...
            Verbose::PrintMess("LBA: " + to_string(spErasedMPs.size()) + " MPs had deleted observations",
                               Verbose::VERBOSITY_DEBUG);
            Verbose::PrintMess("LBA: Current map is " + to_string(pMainKF->GetMap()->GetId()),
                               Verbose::VERBOSITY_DEBUG);
            int numErasedMP = 0;
            ...
            for (KeyFrame *pKFi : spErasedKFs) {
                ...
                Verbose::PrintMess("LBA: Initially KF " + to_string(pKFi->mnId) + " had " + to_string(num_init_MPs) +
                                   ", at the end has " + to_string(num_MPs), Verbose::VERBOSITY_DEBUG);
            }
        }
        for (unsigned int i = 0; i < vpMPs.size(); ++i) {
            MapPoint *pMPi = vpMPs[i];
        ...
        for (KeyFrame *pKFi : vpAdjustKF) {
            ...
            if (numMonoOptPoints + numStereoOptPoints < 50) {
                Verbose::PrintMess(
                        "LBA ERROR: KF " + to_string(pKFi->mnId) + " has only " + to_string(numMonoOptPoints) +
                        " monocular and " + to_string(numStereoOptPoints) + " stereo points", Verbose::VERBOSITY_DEBUG);
            }
            if (dist > 1.0) {
                if (bShowImages) {
                    string strNameFile = pKFi->mNameFile;
                    ...
                    string namefile = "./test_LBA/LBA_KF" + to_string(pKFi->mnId) + "_" +
                                      to_string(numPointsMono + numPointsStereo) + "_D" + to_string(dist) + ".png";
                    cv::imwrite(namefile, imLeft);
                    ...
                    Verbose::PrintMess(
                            "--Number of discarded observations: " + to_string(numMonoBadPoints) + " in mono and " +
                            to_string(numStereoBadPoints) + " in stereo", Verbose::VERBOSITY_DEBUG);
                    Verbose::PrintMess("--To much distance correction in LBA: It has " + to_string(mpObsKFs[pKFi]) +
                                       " observated MPs", Verbose::VERBOSITY_DEBUG);
                    Verbose::PrintMess(
                            "--To much distance correction in LBA: It has " + to_string(mpObsFinalKFs[pKFi]) +
