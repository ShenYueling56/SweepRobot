...
#include "include/orb_slam3/Atlas.h"
#include "include/orb_slam3/Viewer.h"
...
namespace ORB_SLAM3 {
    ...
    void Atlas::CreateNewMap() {
        ...
        mpCurrentMap = new Map(mnLastInitKFidMap); //地图序号加一，关键帧序号也从上一个地图的最后一个关键帧序号加一开始
    ...
    void Atlas::AddRecentInitId(KeyFrame *pKF) {
        mpCurrentMap->AddRecentInitKF(pKF);
    }

    bool Atlas::HasEnoughKFs(long unsigned int curKFId) {
        return mpCurrentMap->HasEnoughKFs(curKFId);
    }
    ...
    void Atlas::EraseMapPoint(MapPoint *pMP) {
        Map *pMapMP = pMP->GetMap();
        pMapMP->EraseMapPoint(pMP);
    }
