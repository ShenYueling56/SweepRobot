...
#include "include/orb_slam3/Map.h"
...
namespace ORB_SLAM3 {
    ...
    vector<KeyFrame *> Map::GetAllOrderedKeyFrames() {
        unique_lock<mutex> lock(mMutexMap);
        vector<KeyFrame *> vpKFs = vector<KeyFrame *>(mspKeyFrames.begin(), mspKeyFrames.end());
        sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lId);
        return vector<KeyFrame *>(vpKFs.begin(), vpKFs.end());
    }
    ...
    bool Map::IsRecentInitKF(KeyFrame *pKF) {
        //return false;
        unique_lock<mutex> lock(mMutexMap);
        return msRecentInitKFs.count(pKF);
    }

    void Map::AddRecentInitKF(KeyFrame *pKF) {
        unique_lock<mutex> lock(mMutexMap);
        mnLastRestartKFid = pKF->mnId;
        msRecentInitKFs.insert(pKF);
    }

    bool Map::HasEnoughKFs(long unsigned int curKFId) {
        unique_lock<mutex> lock(mMutexMap);
        if ((curKFId - mnLastRestartKFid) <= 1)
            return false;
        return true;
    }
...
//        delete *sit;
    ...
    void Map::printReprojectionError(list<KeyFrame *> &lpLocalWindowKFs, KeyFrame *mpCurrentKF, string &name,
                                     string &name_folder) {
