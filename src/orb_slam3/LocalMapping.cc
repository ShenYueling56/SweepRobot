...
#include "include/orb_slam3/LocalMapping.h"
...
namespace ORB_SLAM3 {
    ...
    bool LocalMappingFinished = true;
    ...
    LocalMapping::LocalMapping(System *pSys, Atlas *pAtlas, const float bMonocular, bool bInertial,
        ...
        mpLocalMapTime = new LocalMapTime();
    ...
    void LocalMapping::Run() {
        ...
        while (1) {
            ...
            if (CheckNewKeyFrames() && !mbBadImu) {
                LocalMappingFinished = false;
                mpLocalMapTime->kf_cnt.push_back(mlNewKeyFrames.size());
                mpLocalMapTime->timesCur.push_back(GetTimeNow());

#ifdef LOCAL_BA_TIME_LOGGING
                logCurrentKeyFrame.setNaN();
                logCurrentKeyFrame.frame_time_stamp = mlNewKeyFrames.front()->mTimeStamp;
#endif
                Verbose::PrintMess("Map: new keyframe in.", Verbose::VERBOSITY_SELF_DEBUG);
                ...
                if (!CheckNewKeyFrames()) {
                    ...
                    SearchInNeighbors();
                    Verbose::PrintMess("fuse map point.", Verbose::VERBOSITY_SELF_DEBUG);
                }
                //else
                //    cout<<" keyframe insert too fastly. "<<endl;
                ...
                if (!CheckNewKeyFrames() && !stopRequested()) {
                    //fixme 也许这个是问题所在
                    if (mpAtlas->KeyFramesInMap() > 2 && mpAtlas->HasEnoughKFs(mpCurrentKeyFrame->mnId)) {
                        if (mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized()) {
                            ...
                            timer.tic();
#endif
                            //if(mpCurrentKeyFrame->GetMap()->IsRecentInitKF(mpCurrentKeyFrame))
                            Verbose::PrintMess("start LocalBundleAdjustment.", Verbose::VERBOSITY_SELF_DEBUG);
                            std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
                            Optimizer::LocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(),
                                                             num_FixedKF_BA);
                            std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
                            //if(mpCurrentKeyFrame->GetMap()->IsRecentInitKF(mpCurrentKeyFrame))
                            Verbose::PrintMess("finish LocalBundleAdjustment.", Verbose::VERBOSITY_SELF_DEBUG);
#ifdef LOCAL_BA_TIME_LOGGING
                ...
                float t_procKF = std::chrono::duration_cast<std::chrono::duration<float, std::milli> >(t1 - t0).count();
                float t_MPcull = std::chrono::duration_cast<std::chrono::duration<float, std::milli> >(t2 - t1).count();
                float t_CheckMP = std::chrono::duration_cast<std::chrono::duration<float, std::milli> >(
                        t3 - t2).count();
                float t_searchNeigh = std::chrono::duration_cast<std::chrono::duration<float, std::milli> >(
                        t4 - t3).count();
                float t_Opt = std::chrono::duration_cast<std::chrono::duration<float, std::milli> >(t5 - t4).count();
                float t_KF_cull = std::chrono::duration_cast<std::chrono::duration<float, std::milli> >(
                        t6 - t5).count();
                float t_Insert = std::chrono::duration_cast<std::chrono::duration<float, std::milli> >(t8 - t7).count();
                mpLocalMapTime->procKF.update(t_procKF);
                mpLocalMapTime->MPcull.update(t_MPcull);
                mpLocalMapTime->CheckMP.update(t_CheckMP);
                mpLocalMapTime->searchNeigh.update(t_searchNeigh);
                mpLocalMapTime->Opt.update(t_Opt);
                mpLocalMapTime->KF_cull.update(t_KF_cull);
                mpLocalMapTime->Insert.update(t_Insert);
                //DEBUG--
                /*f_lm << setprecision(6);
                f_lm << t_procKF << ",";
                f_lm << t_MPcull << ",";
                ...
                //--
                LocalMappingFinished = true;
                Verbose::PrintMess("Map run finish", Verbose::VERBOSITY_SELF_DEBUG);
        ...
        SetFinish();
        cout << " LocalMapping Run finish " << endl;
    }
...
//加速关键，fuse操作比较耗时，减少vpTargetKFs数量
    void LocalMapping::SearchInNeighbors() {
//    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
        // Retrieve neighbor keyframes
#ifdef LIMIT_LOCAL_BA
        int nn = GOOD_GRAPH_NN_KF;
#else
        int nn = 10;
#endif
        if (mbMonocular)
...
//    std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
//    cout<<" 2 "<<std::chrono::duration_cast<std::chrono::duration<float,std::milli> >(t2 - t1).count();
        // Add some covisible of covisible
        // Extend to some second neighbors if abort is not requested
        for (int i = 0, imax = vpTargetKFs.size(); i < imax; i++) {
            const vector<KeyFrame *> vpSecondNeighKFs = vpTargetKFs[i]->GetBestCovisibilityKeyFrames(
                    nn);//default 20 whatever mono or stereo
...
//    std::chrono::steady_clock::time_point t3 = std::chrono::steady_clock::now();
//    cout<<" 3 "<<std::chrono::duration_cast<std::chrono::duration<float,std::milli> >(t3 - t2).count();
        ...
        // Search matches by projection from current KF in target KFs / accelerate Computationally expensive
        ORBmatcher matcher;
...
//    std::chrono::steady_clock::time_point t4 = std::chrono::steady_clock::now();
//    cout<<" 4 "<<std::chrono::duration_cast<std::chrono::duration<float,std::milli> >(t4 - t3).count();
        if (mbAbortBA)
            return;
...
//    cout<<" 5 "<<std::chrono::duration_cast<std::chrono::duration<float,std::milli> >(t5 - t4).count();
        matcher.Fuse(mpCurrentKeyFrame, vpFuseCandidates);
        if (mpCurrentKeyFrame->NLeft != -1) matcher.Fuse(mpCurrentKeyFrame, vpFuseCandidates, true);

        // Update points accelerate Computationally expensive
...
//    cout<<" 6 "<<std::chrono::duration_cast<std::chrono::duration<float,std::milli> >(t6 - t5).count();
        // Update connections in covisibility graph
        mpCurrentKeyFrame->UpdateConnections();
//    std::chrono::steady_clock::time_point t7 = std::chrono::steady_clock::now();
//    cout<<" 7 "<<std::chrono::duration_cast<std::chrono::duration<float,std::milli> >(t7 - t6).count();
//    cout<<endl;
    }
    ...
    void LocalMapping::KeyFrameCulling() {
        ...
        float redundant_th = REDUNDANT_TH;
//        if (!mbInertial)
//            redundant_th = 0.9;
//        else if (mbMonocular)
//            redundant_th = 0.9;
...
//            redundant_th = 0.5;
        ...
        for (vector<KeyFrame *>::iterator vit = vpLocalKeyFrames.begin(), vend = vpLocalKeyFrames.end();
            ...
            if ((pKF->mnId == pKF->GetMap()->GetInitKFid()) || pKF->isBad() || pKF->GetMap()->IsRecentInitKF(pKF))
                continue;
                    ...
                    if (pKF->mPrevKF && pKF->mNextKF) {
                        ...
                        if ((bInitImu && (pKF->mnId < last_ID) && t < 3.) || (t < 0.5)) {
                            ...
                            pKF->SetBadFlag();
                                   ...
                                   (cv::norm(pKF->GetImuPosition() - pKF->mPrevKF->GetImuPosition()) < 0.02) &&
                                   (t < 3)) {
                            pKF->mNextKF->mpImuPreintegrated->MergePrevious(pKF->mpImuPreintegrated);
                            pKF->mNextKF->mPrevKF = pKF->mPrevKF;
                            pKF->mPrevKF->mNextKF = pKF->mNextKF;
    ...
    void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA) {
        ...
        if (!mpCurrentKeyFrame->GetMap()->isImuInitialized()) {
            ...
            for (vector<KeyFrame *>::iterator itKF = vpKF.begin(); itKF != vpKF.end(); itKF++) {
                ...
                cv::Mat _vel = ((*itKF)->GetImuPosition() - (*itKF)->mPrevKF->GetImuPosition()) /
        ...
        cout << "ba: " << mpCurrentKeyFrame->GetAccBias() << endl;
        cout << "bg: " << mpCurrentKeyFrame->GetGyroBias() << endl;
        float t_inertial_only = std::chrono::duration_cast<std::chrono::duration<float> >(t1 - t0).count();
        float t_update = std::chrono::duration_cast<std::chrono::duration<float> >(t3 - t2).count();
        float t_viba = std::chrono::duration_cast<std::chrono::duration<float> >(t5 - t4).count();
