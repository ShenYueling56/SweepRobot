...
#include "include/orb_slam3/KeyFrameDatabase.h"
...
namespace ORB_SLAM3 {
    ...
    KeyFrameDatabase::KeyFrameDatabase(const ORBVocabulary &voc) :
            mpVoc(&voc) {
        mvInvertedFile.resize(VOCProsessor::size(voc));
    }


    void KeyFrameDatabase::add(KeyFrame *pKF) {
        unique_lock<mutex> lock(mMutex);
        for (BowVec::const_iterator vit = pKF->mBowVec.begin(), vend = pKF->mBowVec.end(); vit != vend; vit++) {
            //cout<<"vit->first: "<<vit->first<<endl;
    ...
    void KeyFrameDatabase::clear() {
        ...
        mvInvertedFile.resize(VOCProsessor::size(mpVoc));
    }
    ...
    vector<KeyFrame *> KeyFrameDatabase::DetectLoopCandidates(KeyFrame *pKF, float minScore) {
        ...
        for (list<KeyFrame *>::iterator lit = lKFsSharingWords.begin(), lend = lKFsSharingWords.end();
            ...
            if (pKFi->mnLoopWords > minCommonWords) {
                ...
                float si = VOCProsessor::score(mpVoc, pKF->mBowVec, pKFi->mBowVec);
    ...
    vector<KeyFrame *> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F, Map *pMap) {
        ...
        // Search all keyframes that share a word with current frame
        {
            ...
            for (BowVec::const_iterator vit = F->mBowVec.begin(), vend = F->mBowVec.end(); vit != vend; vit++) {
                list<KeyFrame *> &lKFs = mvInvertedFile[vit->first];
        ...
        for (list<KeyFrame *>::iterator lit = lKFsSharingWords.begin(), lend = lKFsSharingWords.end();
            ...
            if (pKFi->mnRelocWords > minCommonWords) {
                nscores++;
                float si = VOCProsessor::score(mpVoc, F->mBowVec, pKFi->mBowVec);
                pKFi->mRelocScore = si;
                lScoreAndMatch.push_back(make_pair(si, pKFi));
            }
        }
    ...
    void KeyFrameDatabase::PreSave() {
        ...
        for (int i = 0, numEl = mvInvertedFile.size(); i < numEl; ++i) {
            for (std::list<KeyFrame *>::const_iterator it = mvInvertedFile[i].begin(), end = mvInvertedFile[i].end();
                 it != end; ++it) {
