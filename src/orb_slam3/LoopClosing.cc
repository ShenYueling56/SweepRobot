...
#include "include/orb_slam3/LoopClosing.h"

#include "include/orb_slam3/Sim3Solver.h"
...
namespace ORB_SLAM3 {
    ...
    LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale) :
        ...
        mpLoopCloseTime = new LoopCloseTime();
    }
    ...
    void LoopClosing::Run() {
        ...
        while (1) {
            ...
            if (CheckNewKeyFrames()) {
                ...
                Verbose::PrintMess("LoopClosing in", Verbose::VERBOSITY_SELF_DEBUG);
                if (mpLastCurrentKF) {
                ...
                if (NewDetectCommonRegions()) {
                    ...
                    Verbose::PrintMess("NewDetectCommonRegions", Verbose::VERBOSITY_SELF_DEBUG);
...
//                        cout << "scale w2w1: " << mSold_new.scale() << endl;
                    ...
                    if (mbLoopDetected) {
                        LoopClosingFinished = false;
                        vdPR_CurrentTime.push_back(mpCurrentKF->mTimeStamp);
                        vdPR_MatchedTime.push_back(mpLoopMatchedKF->mTimeStamp);
                        vnPR_TypeRecogn.push_back(0);


                        Verbose::PrintMess("\n*Loop detected", Verbose::VERBOSITY_QUIET);
                    ...
                    LoopClosingFinished = true;

                    t3 = std::chrono::steady_clock::now();

                    double t_Detect = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                            t1 - t0).count();
                    double t_Merge = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                            t2 - t1).count();
                    double t_Loop = std::chrono::duration_cast<std::chrono::duration<double, std::milli> >(
                            t3 - t2).count();
                    mpLoopCloseTime->Detect.update(t_Detect);
                    mpLoopCloseTime->Merge.update(t_Merge);
                    mpLoopCloseTime->Loop.update(t_Loop);
                    mpLoopCloseTime->timesCur.push_back(GetTimeNow());
                }
                Verbose::PrintMess("LoopClosing end", Verbose::VERBOSITY_SELF_DEBUG);
        ...
        SetFinish();
        cout << " LoopClosing Run finish " << endl;
    }
    ...
    bool LoopClosing::NewDetectCommonRegions() {
        ...
        //cout<<"close loop 1    "<<mnLoopNumCoincidences<<endl;
        ...
        //cout<<"close loop 2    "<<mnMergeNumCoincidences<<endl;
...
/*#ifdef COMPILEDWITHC11
        ...
        /*if(mpTracker->mSensor==System::IMU_MONOCULAR ||mpTracker->mSensor==System::IMU_STEREO)
    {
        ...
        int index = 0;
        //cout << "vpBowCand "<<vpBowCand.size() << endl;
        for (KeyFrame *pKFi : vpBowCand) {
            //cout << endl << "-------------------------------" << endl;
            //fixme fix bug add pKFi->GetVectorCovisibleKeyFrames().empty() for vpCovKFi.push_back(vpCovKFi[0]);
            if (!pKFi || pKFi->isBad() || pKFi->GetVectorCovisibleKeyFrames().empty())
                continue;

            //cout << "step 1 nNumCovisibles "<<nNumCovisibles<< endl;

            // Current KF against KF with covisibles version
            std::vector<KeyFrame *> vpCovKFi = pKFi->GetBestCovisibilityKeyFrames(nNumCovisibles);
            //cout << "step 2 "<< endl;
            //cout << "vpCovKFi "<< vpCovKFi.size()<< endl;
...
//        f_succes_pr << "% NumCand: " << std::to_string(numCandidates) << "; matches: " << std::to_string(maxMatched) << endl;
    ...
    void LoopClosing::CorrectLoop() {
        ...
        if (!pLoopMap->isImuInitialized() || (pLoopMap->KeyFramesInMap() < GLOABL_BA_KF && mpAtlas->CountMaps() == 1)) {
            mbRunningGBA = true;
            mbFinishedGBA = false;
            mbStopGBA = false;
    ...
    void LoopClosing::MergeLocal() {
        ...
        Verbose::PrintMess(
                "MERGE-VISUAL: Initially there are " + to_string(pCurrentMap->KeyFramesInMap()) + " KFs and " +
                to_string(pCurrentMap->MapPointsInMap()) + " MPs in the active map ", Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("MERGE-VISUAL: Initially there are " + to_string(pMergeMap->KeyFramesInMap()) + " KFs and " +
                           to_string(pMergeMap->MapPointsInMap()) + " MPs in the matched map ",
                           Verbose::VERBOSITY_DEBUG);
        ...
        Verbose::PrintMess("MERGE-VISUAL: Initial number of KFs in local window from active map: " +
                           to_string(spLocalWindowKFs.size()), Verbose::VERBOSITY_DEBUG);
        ...
        Verbose::PrintMess("MERGE-VISUAL: Last number of KFs in local window from the active map: " +
                           to_string(spLocalWindowKFs.size()), Verbose::VERBOSITY_DEBUG);
        ...
        Verbose::PrintMess(
                "MERGE-VISUAL: Number of MPs in local window from active map: " + to_string(spLocalWindowMPs.size()),
                Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess(
                "MERGE-VISUAL: Number of MPs in the active map: " + to_string(pCurrentMap->GetAllMapPoints().size()),
                Verbose::VERBOSITY_DEBUG);
        ...
        Verbose::PrintMess("MERGE-VISUAL: Initial number of KFs in the local window from matched map: " +
                           to_string(spMergeConnectedKFs.size()), Verbose::VERBOSITY_DEBUG);
        nNumTries = 0;
        ...
        Verbose::PrintMess("MERGE-VISUAL: Last number of KFs in the localwindow from matched map: " +
                           to_string(spMergeConnectedKFs.size()), Verbose::VERBOSITY_DEBUG);
        ...
        if (mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO) {
            ...
            Verbose::PrintMess("MERGE-VISUAL: Local current window->" + to_string(vpLocalCurrentWindowKFs.size()) +
                               "; Local merge window->" + to_string(vpMergeConnectedKFs.size()),
                               Verbose::VERBOSITY_DEBUG);
        ...
        if (vpCurrentMapKFs.size() == 0) {
            ...
            Verbose::PrintMess("MERGE-VISUAL: Calculate the new position of the elements outside of the window",
                               Verbose::VERBOSITY_DEBUG);
            //Apply the transformation
            {
                if (mpTracker->mSensor == System::MONOCULAR) {
                    ...
                    for (KeyFrame *pKFi : vpCurrentMapKFs) {
                        ...
                        if (pCurrentMap->isImuInitialized()) {
                            Eigen::Matrix3d Rcor =
                                    eigR.transpose() * vNonCorrectedSim3[pKFi].rotation().toRotationMatrix();
                            pKFi->SetVelocity(
                                    Converter::toCvMat(Rcor) * pKFi->GetVelocity()); // TODO: should add here scale s
                        }
                    ...
                    for (MapPoint *pMPi : vpCurrentMapMPs) {
                        ...
                        Eigen::Matrix<double, 3, 1> eigCorrectedP3Dw = g2oCorrectedSwi.map(
                                g2oNonCorrectedSiw.map(eigP3Dw));
            ...
            Verbose::PrintMess("MERGE-VISUAL: Apply transformation to all elements of the old map",
                               Verbose::VERBOSITY_DEBUG);
...

            {
                ...
                Verbose::PrintMess(
                        "MERGE-VISUAL: There are " + to_string(pMergeMap->KeyFramesInMap()) + " KFs in the map",
                        Verbose::VERBOSITY_DEBUG);
                Verbose::PrintMess(
                        "MERGE-VISUAL: It will be inserted " + to_string(vpCurrentMapKFs.size()) + " KFs in the map",
                        Verbose::VERBOSITY_DEBUG);
                ...
                Verbose::PrintMess(
                        "MERGE-VISUAL: There are " + to_string(pMergeMap->MapPointsInMap()) + " MPs in the map",
                        Verbose::VERBOSITY_DEBUG);
                Verbose::PrintMess(
                        "MERGE-VISUAL: It will be inserted " + to_string(vpCurrentMapMPs.size()) + " MPs in the map",
                        Verbose::VERBOSITY_DEBUG);
        ...
        Verbose::PrintMess("MERGE-VISUAL: Finally there are " + to_string(pMergeMap->KeyFramesInMap()) + "KFs and " +
                           to_string(pMergeMap->MapPointsInMap()) + " MPs in the complete map ",
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("MERGE-VISUAL:Completed!!!!!", Verbose::VERBOSITY_DEBUG);

        if (bRelaunchBA &&
            (!pCurrentMap->isImuInitialized() || (pCurrentMap->KeyFramesInMap() < GLOABL_BA_KF && mpAtlas->CountMaps() == 1))) {
    ...
    void LoopClosing::printReprojectionError(set<KeyFrame *> &spLocalWindowKFs, KeyFrame *mpCurrentKF, string &name) {
        ...
        for (KeyFrame *pKFi : spLocalWindowKFs) {
            ...
            string filename_img =
                    path_imgs + "KF" + to_string(mpCurrentKF->mnId) + "_" + to_string(pKFi->mnId) + name + "points" +
