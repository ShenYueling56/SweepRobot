...
namespace ORB_SLAM3 {
    ...
    KeyFrame::KeyFrame() :
            ...
            mbToBeErased(false), mbBad(false), mHalfBaseline(0), mbCurrentPlaceRecognition(false), mbHasHessian(false),
...
//    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));
    ...
    void KeyFrame::ComputeBoW() {
        if (mBowVec.empty() || mFeatVec.empty()) {
            ...
            VOCProsessor::transform(mpORBvocabulary, mDescriptors, mBowVec, mFeatVec, 4);
        }
    }
...
// 线程错误发生地
    ...
    void KeyFrame::UpdateBestCovisibles() {
        //cout<<"UpdateBestCovisibles"<<endl;
        unique_lock<mutex> lock(mMutexConnections);
    ...
    set<KeyFrame *> KeyFrame::GetConnectedKeyFrames() {
        //cout<<"GetConnectedKeyFrames"<<endl;
        unique_lock<mutex> lock(mMutexConnections);
        set<KeyFrame *> s;
    ...
    vector<KeyFrame *> KeyFrame::GetVectorCovisibleKeyFrames() {
        //cout<<"GetVectorCovisibleKeyFrames"<<endl;
        unique_lock<mutex> lock(mMutexConnections);
        return mvpOrderedConnectedKeyFrames;
    }

    vector<KeyFrame *> KeyFrame::GetBestCovisibilityKeyFrames(const int &N) {
        //cout<<"GetBestCovisibilityKeyFrames "<<(int)mvpOrderedConnectedKeyFrames.size()<<" "<<N<<endl;
        unique_lock<mutex> lock(mMutexConnections);
    ...
    vector<KeyFrame *> KeyFrame::GetCovisiblesByWeight(const int &w) {
        //cout<<"GetCovisiblesByWeight"<<endl;
        unique_lock<mutex> lock(mMutexConnections);
...
//    cerr<<"UpdateConnections 1"<<endl;
        vector<MapPoint *> vpMP;
        {
            unique_lock<mutex> lockMPs(mMutexFeatures);
            vpMP = mvpMapPoints;
        }
//    cerr<<"UpdateConnections 2"<<endl;
...
//        cerr<<"observations over"<<endl;
        }
//    cerr<<"UpdateConnections 3"<<endl;

        //为其单独建立父节点
        if (mpMap->IsRecentInitKF(this)) {
            vector<KeyFrame *> orderedKFsInMap = mpMap->GetAllOrderedKeyFrames();
            vector<KeyFrame *>::iterator ret;
            ret = std::find(orderedKFsInMap.begin(), orderedKFsInMap.end(), this);
            if (ret == orderedKFsInMap.end()) {
                cout << "UpdateConnections not found this in Map!" << endl;
            } else {
                if (ret != orderedKFsInMap.begin()) {
                    for (vector<KeyFrame *>::iterator iter = ret; iter != orderedKFsInMap.begin(); iter--) {
                        //获取其上一帧
                        KeyFrame *pKFn = *(iter - 1);
                        if (!pKFn->isBad() && pKFn->mnId < mnId) {
                            //剔除原来的
                            if (mpParent)
                            ...
                            if (!mpMap->IsRecentInitKF(mpParent))
                                break;
                        }
                    }
                }
...
//    cerr<<"UpdateConnections 4"<<endl;

        //cout<<"KFcounter.empty()?"<<endl;
...
//        }
//fixme restart
            if (mbFirstConnection && mnId != mpMap->GetInitKFid() && !mpMap->IsRecentInitKF(this)) {
                ...
                //fixme 构建essential tree
                mpParent = mvpOrderedConnectedKeyFrames.front();
                mpParent->AddChild(this);
                mbFirstConnection = false;
            }
            //没用，甚至会有错误，自己加的
            //else if(mbFirstConnection && mpMap->IsRecentInitKF(mnId) && mpMap->KeyFramesInMap()>1)
            //{
            //    mpParent = mpMap->GetAllKeyFrames()[mpMap->KeyFramesInMap()-2];
            //    mpParent->AddChild(this);
            //    mbFirstConnection = false;
            //}
...
//        mpParent->EraseChild(this);
    ...
    void KeyFrame::SetBadFlag() {
        ...
        {
            unique_lock<mutex> lock(mMutexConnections);
            if (mnId == mpMap->GetInitKFid() || mpMap->IsRecentInitKF(this)) {
